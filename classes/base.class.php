...
abstract class PrisnaBWTItem {
	...
	public function render($_options, $_html_encode=false) {
		...
		$result = PrisnaBWTCommon::renderObject($this, $_options, $_html_encode);
	...
	protected function _json() {
		...
		foreach ($this as $property => $value)
			if (!PrisnaBWTCommon::endsWith($property, '_json'))
				$fields[] = $property;

		foreach ($fields as $value)
			if (substr($value, 0, 1) != '_')
				$this->setProperty($value . '_json', PrisnaBWTCommon::jsonCompatible($this->getProperty($value)));

	}

}

abstract class PrisnaBWTField {
	...
	public function satisfyDependence($_fields) {
		...
		if (PrisnaBWTValidator::isEmpty($this->dependence))
			return;

		$this->_dependence = PrisnaBWTCommon::getArrayItems($this->dependence, $_fields);
	...
	protected function _has_dependence() {
		...
		return !is_null($this->dependence) && !PrisnaBWTValidator::isEmpty($this->dependence);
	...
	protected function _dependence_show() {
		...
		foreach ($this->_dependence as $field)
			if (PrisnaBWTCommon::inArray($field->value, $this->dependence_show_value))
				$result[] = $field->id;
	...
	protected function _render($_options, $_html_encode) {
		...
		$result = PrisnaBWTCommon::renderObject($this, $options, $_html_encode);
...
class PrisnaBWTColorField extends PrisnaBWTField {
...
class PrisnaBWTTextField extends PrisnaBWTField {
...
class PrisnaBWTPremiumField extends PrisnaBWTField {

	public $images_path;

	public function output($_html_encode=false) {

		$this->images_path = PRISNA_BWT__IMAGES;
...
class PrisnaBWTUsageField extends PrisnaBWTField {
...
class PrisnaBWTHeadingField extends PrisnaBWTField {
	...
	public function output($_html_encode=false) {
		...
		$result = parent::render(array(
			...
			'meta_tag_rules' => array(
				...
				array(
					'expression' => !PrisnaBWTValidator::isEmpty($this->description_message),
					'tag' => 'description'
				)
			)
		), $_html_encode);
...
class PrisnaBWTHeading2Field extends PrisnaBWTField {
...
class PrisnaBWTSitemapviewField extends PrisnaBWTField {
	...
	public function __construct($_properties) {
		...
		$this->xml_url = PrisnaBWTCommon::getHomeUrl('/' . PrisnaBWTConfig::getSitemapFilename() . '.xml');
		$this->csv_url = PrisnaBWTCommon::getHomeUrl('/' . PrisnaBWTConfig::getSitemapFilename() . '.csv');
...
class PrisnaBWTRuleField extends PrisnaBWTField {
	...
	public function __construct($_properties) {
		...
		$this->priority = new PrisnaBWTSelectField(array(
			'title_message' => '',
			'description_message' => '',
		...
		$this->changefreq = new PrisnaBWTSelectField(array(
			'title_message' => '',
			'description_message' => '',
			'id' => $this->id . '_changefreq',
			'type' => 'select',
			'values' => array(
				'always' => PrisnaBWTMessage::get('changefreq_always'),
				'hourly' => PrisnaBWTMessage::get('changefreq_hourly'),
				'daily' => PrisnaBWTMessage::get('changefreq_daily'),
				'weekly' => PrisnaBWTMessage::get('changefreq_weekly'),
				'monthly' => PrisnaBWTMessage::get('changefreq_monthly'),
				'yearly' => PrisnaBWTMessage::get('changefreq_yearly'),
				'never' => PrisnaBWTMessage::get('changefreq_never')
			),
...
class PrisnaBWTToggleField extends PrisnaBWTField {
...
class PrisnaBWTRangeField extends PrisnaBWTField {
...
class PrisnaBWTTextareaField extends PrisnaBWTField {
...
class PrisnaBWTExportField extends PrisnaBWTField {
...
class PrisnaBWTLanguageOptionField extends PrisnaBWTField {
	...
	public function __construct($_properties) {
		...
		$this->base_url = PRISNA_BWT__IMAGES . '/';
...
class PrisnaBWTLanguageOrderOptionField extends PrisnaBWTField {
...
class PrisnaBWTLanguageField extends PrisnaBWTField {
	...
	protected function _set_options() {
		...
		for ($i=0; $i<$this->columns; $i++)
			$this->_collections[$i] = new PrisnaBWTItemCollection();
		...
		foreach ($this->values as $key => $value) {
			...
			$this->_collections[$group]->add(new PrisnaBWTLanguageOptionField((object) array(
				'id' => PrisnaBWTCommon::cleanId($this->id . '_' . $key, '_'),
				'name' => $this->id,
	...
	protected function _set_order_options() {
		...
		$this->_collection_order = new PrisnaBWTItemCollection();
		...
		foreach ($this->values as $key => $value) {
			...
			$items[array_search($key, $values)] = new PrisnaBWTLanguageOrderOptionField((object) array(
				'id' => PrisnaBWTCommon::cleanId($this->id . '_order_' . $key, '_'),
				'option' => $key,
				'value' => $value
			));
...
class PrisnaBWTCheckboxOptionField extends PrisnaBWTField {
...
class PrisnaBWTCheckboxField extends PrisnaBWTField {
	...
	protected function _set_options() {
		...
		$this->collection = new PrisnaBWTItemCollection();

		foreach ($this->values as $key => $value) {

			$this->collection->add(new PrisnaBWTCheckboxOptionField((object) array(
				'id' => PrisnaBWTCommon::cleanId($this->id . '_' . $key, '_'),
				'name' => $this->id,
				'checked' => is_array($this->value) ? in_array((string) $key, $this->value, true) : false,
				'option' => $key,
...
class PrisnaBWTExcategoryField extends PrisnaBWTExclitemField {
...
class PrisnaBWTExpageField extends PrisnaBWTExclitemField {
...
class PrisnaBWTExpostField extends PrisnaBWTExclitemField {
...
class PrisnaBWTExclitemOptionField extends PrisnaBWTField {
...
class PrisnaBWTExclitemField extends PrisnaBWTCheckboxField {
	...
	protected function _sort() {
		...
		$result = array();
		$temp = PrisnaBWTCommon::chain('id', 'parent', 'value', $this->values);
	...
	protected function _set_options() {
		...
		foreach ($this->values as $key => $value) {
			...
			$this->collection->add(new PrisnaBWTExclitemOptionField((object) array(
				'id' => PrisnaBWTCommon::cleanId($this->id . '_' . $key, '_'),
				'name' => $this->id,
				'checked' => is_array($this->value) ? in_array((string) $key, $this->value, true) : false,
				'option' => $key,
...
class PrisnaBWTRadioOptionField extends PrisnaBWTField {
...
class PrisnaBWTRadioField extends PrisnaBWTField {
	...
	protected function _set_options() {
		...
		foreach ($this->values as $key => $value) {
			...
			$this->collection->add(new PrisnaBWTRadioOptionField((object) array(
				'id' => $this->id . '_' . $key,
				'name' => $this->id,
...
class PrisnaBWTVisualOptionField extends PrisnaBWTField {
...
class PrisnaBWTVisualField extends PrisnaBWTField {
	...
	protected function _set_options() {
		...
		foreach ($this->values as $key => $value) {
			...
			$this->collection->add(new PrisnaBWTVisualOptionField((object) array(
				'id' => $this->id . '_' . $key,
				'name' => $this->id,
				'checked' => $this->value == $key,
				'option' => $key,
...
class PrisnaBWTSelectOptionField extends PrisnaBWTField {
...
class PrisnaBWTSelectField extends PrisnaBWTField {
	...
	protected function _set_options() {
		...
		foreach ($this->values as $key => $value) {
			...
			$this->collection->add(new PrisnaBWTSelectOptionField((object) array(
	...
	public function output($_template='select.tpl', $_html_encode=false) {
		...
		$result = parent::render(array(
			...
			'meta_tag_rules' => array(
				array(
					'expression' => property_exists($this, 'post_id') && !PrisnaBWTValidator::isEmpty($this->post_id),
					'tag' => 'has_post_id'
				)
			)
		), $_html_encode);
...
class PrisnaBWTPermalinksField extends PrisnaBWTField {
	...
	protected function _initialize_resource_field($_properties) {
		...
		$_resource_field['site_url'] = PrisnaBWTCommon::getSiteHost();
		$_resource_field['values'] = $this->_get_resource_values();
		
		$this->_resource_field = new PrisnaBWTSelectField($_resource_field);
	...
	protected function _get_files_scopes() {
		...
		$contents = PrisnaBWTFileHandler::read($file);
		...
		$from = PrisnaBWTConfig::getSettingValue('from');
...
class PrisnaBWTTranslationsField extends PrisnaBWTField {
	...
	public static function getFilesPaths($_folder, $_globals=false) {
		...
		foreach ($contents as $name) {
			$file = $_folder . '/' . $name;
			if (PrisnaBWTCommon::endsWith($name, '.xml') && !PrisnaBWTCommon::startsWith($name, 'permalinks') && !PrisnaBWTCommon::startsWith($name, 'log_') && @is_file($file) && ($_globals || !PrisnaBWTCommon::endsWith($name, 'global.xml')))
				$result[] = $file;
		}
...
class PrisnaBWTSeologField extends PrisnaBWTField {
	...
	public function __construct($_properties) {
		...
		parent::__construct($_properties);
		$this->_file = PrisnaBWTCommon::getLogFile(PrisnaBWTCommon::getTestApiKey());
		$this->empty_message = PrisnaBWTMessage::get('seo_log_empty_message');
	...
	protected function _render_log($_template='log.tpl', $_html_encode=false) {
		...
		for ($i=$count-1; $i>=0; $i--) {
			...
			$result .= PrisnaBWTCommon::renderObject((object) $entry, array(
				'type' => 'file',
				'content' => '/admin/' . $_template,
				'meta_tag_rules' => array(
					array(
	...
	protected function _get_log() {
		...
		$contents = PrisnaBWTFileHandler::read($this->_file);
...
class PrisnaBWTItemCollection {
	...
	public function render($_options, $_html_encode=false) {
		...
		$result = PrisnaBWTCommon::renderObject($object, $_options, $_html_encode);
